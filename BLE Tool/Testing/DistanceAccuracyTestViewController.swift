//
//  DistanceAccuracyTestViewController.swift
//  BLE Tool
//
...
class DistanceAccuracyTestViewController: UIViewController, CLLocationManagerDelegate {
    ...
    var capturedDataPoints: [[DistanceReadings]] = []
    ...
    var chartsVC : DistanceChartViewController?
    ...
    func locationManager(_ manager: CLLocationManager, didRange beacons: [CLBeacon], satisfying beaconConstraint: CLBeaconIdentityConstraint) {
        ...
        distanceLabel.text = "Distance: \(round(beacons.first!.accuracy * 100)/100)m"
        
        //Get the current distance
        var currentDistance = testDistances[currentTestDistanceIndex]
        
        //Create the reading and add it
        let reading = DistanceReadings(time: Date(), distance: abs(Float(beacons.first!.accuracy) - Float(currentDistance)))
        ...
        if capturedDataPoints[currentTestDistanceIndex].count < dataPoints {
            ...
            for dataPoint in self.capturedDataPoints[currentTestDistanceIndex] {
                totalDistance += Double(dataPoint.distance)
            }
            
            var averageDistance = 0.0
            if totalDistance != 0.0 {
                averageDistance = totalDistance/Double(self.capturedDataPoints[currentTestDistanceIndex].count)
            }
            
            self.distanceLabel.text = self.distanceLabel.text! + " Average Error: \(round(averageDistance * 100)/100)m"
    ...
    func testFinished(){
        ...
        var fileName = timeFormatted + "-" + "DistanceAccuracy" + "-" + beaconName
        ...
        for i in 0...dataPoints - 1 {
            ...
            for j in 0...testDistances.count - 1 {
                dataPointData.setObject(capturedDataPoints[j][i].distance, forKey: "\(testDistances[j])m" as NSCopying);
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "chartsSegue"{
            let destination = segue.destination as! DistanceChartViewController
